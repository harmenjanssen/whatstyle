...
url: /articles/git-branches-workflow
norday: true
---
...
At GRRR we use the [OneFlow model](http://endoflineblog.com/oneflow-a-git-branching-model-and-workflow) for new projects, but we support legacy projects using the Git Flow model as described in [A Successful Git Branching Model](http://nvie.com/posts/a-successful-git-branching-model/).
...
What is a good way to manage feature branches? And in a broader sense, what do you want your Git history to look like?
...
I want to answer the question _"What was intended here?"_.
...
Note that there's no hard and fast rule. You, the developer has to decide which commits are individually interesting to retain for future reference.
Most of the time though, I think a handful commits will suffice.
...
- Your branch gets accepted and merged into `master` (We use GitHub's _Rebase and merge_, but use whichever you prefer)
...
Git will squash the middle three commits into the top one, and discard their commit messages.
...
💡 Pro-tip: grab this git alias to drop into interactive rebase starting at the first commit of your branch: [https://github.com/harmenjanssen/dotfiles/blob/master/gitconfig#L49](https://github.com/harmenjanssen/dotfiles/blob/master/gitconfig#L49)
...
A way to ensure this is the `--ff-only` flag. This ensures a branch is up-to-date before it can be merged into `master`.
...
💡 This corresponds to GitHub's _Rebase and merge_ option when merging a Pull Request in its web interface.
...
When rebasing interactively, you can change the order of the lines to change commit order.
Just remember that you might conflict with yourself when you change into an impossible order. (for instance when a commit references code from a previous commit)

#### Auto-squash and auto-fixup

`git commit` supports `--squash` and `--fixup` flags. They will mark commits as such when rebasing interactively.
